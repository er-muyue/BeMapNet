import torch
from cd import chamfer_distance
...
def line_matching_by_CD(inst_pred_lines, inst_pred_confidence, inst_label_lines, threshold):
    ...
    for i, key_pred in enumerate(inst_pred_lines_keys):
        for j, key_label in enumerate(inst_label_lines_keys):
            CD[i, j] = chamfer_distance(
                inst_pred_lines[key_pred][None],
                inst_label_lines[key_label][None],
                bidirectional=True,
                threshold=threshold,
            )
    ...
    if pred_num > 0 and label_num > 0:
        while True:
            idx = torch.argmin(CD)
            i = torch.div(idx, CD.shape[1], rounding_mode="floor")
            j = (idx % CD.shape[1]).item()
    ...
    for n in range(N):
        for c in range(C):
            ...
            if len(inst_pred_lines) == 0 and len(inst_label_lines) == 0:
                continue
            AP_matrix[c] += single_instance_line_AP(
                inst_pred_lines, confidence[n], inst_label_lines, thresholds, sampled_recalls=sampled_recalls
            )
            AP_count_matrix[c] += 1
...
def single_instance_line_AP(inst_pred_lines, inst_pred_confidence, inst_label_lines, thresholds, sampled_recalls):
    ...
    for t in range(num_thres):
        ...
        precision, recall = get_precision_recall_curve_by_confidence(
            matching_list, len(inst_label_lines), thresholds[t]
        )
...
def get_precision_recall_curve_by_confidence(matching_list, num_gt, threshold):
    ...
    TP = torch.tensor(TP[1:]).cuda()
    FP = torch.tensor(FP[1:]).cuda()
