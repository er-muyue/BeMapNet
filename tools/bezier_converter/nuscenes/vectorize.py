...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples(self, location, ego2global_translation, ego2global_rotation):
        map_pose = ego2global_translation[:2]       # T
        rotation = Quaternion(ego2global_rotation)  # R
    ...
    def get_map_geom(self, patch_box, patch_angle, layer_names, location):
        ...
        for layer_name in layer_names:
            if layer_name in self.line_classes:
                map_explorer_x = self.map_explorer[location]
                geoms = map_explorer_x._get_layer_line(patch_box, patch_angle, layer_name)
            elif layer_name in self.polygon_classes:
                map_explorer_x = self.map_explorer[location]
                geoms = map_explorer_x._get_layer_polygon(patch_box, patch_angle, layer_name)
